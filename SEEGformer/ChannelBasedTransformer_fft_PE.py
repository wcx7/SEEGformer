...
class ChannelBasedTransformer_fft(nn.Module):
    def __init__(self, num_heads, fft_dim, embed_dim, mlp_ratio, dropout, n_chans, num_blocks, num_classes, position_encoding, pool = 'mean'):
        ...
        self.transformer_blocks_real = nn.ModuleList([TransformerBlock(embed_dim, num_heads, mlp_ratio, dropout) for _ in range(num_blocks)])
        self.transformer_blocks_imag = nn.ModuleList([TransformerBlock(embed_dim, num_heads, mlp_ratio, dropout) for _ in range(num_blocks)])
        self.transformer_blocks_abs = nn.ModuleList([TransformerBlock(embed_dim, num_heads, mlp_ratio, dropout) for _ in range(num_blocks)])
        

        self.pos_embed = nn.Parameter(torch.zeros(1, n_chans + 1, position_encoding.shape[1]), requires_grad=False)  # 位置编码
        ...
        self.cls_weight_real = nn.Parameter(0.3333 * torch.ones(1,), requires_grad=True)
        self.cls_weight_imag = nn.Parameter(0.3333 * torch.ones(1,), requires_grad=True)
        self.cls_weight_abs = nn.Parameter(0.3333 * torch.ones(1,), requires_grad=True)
       
        self.classifier_real = nn.Linear(embed_dim, num_classes)
        self.classifier_imag = nn.Linear(embed_dim, num_classes)
        self.classifier_abs = nn.Linear(embed_dim, num_classes)
    ...
    def forward(self, x_real, x_imag, x_abs):
        ...
        x_real = x_real + self.pos_embed_real[:, 1:, :]     # 和位置编码相加
        ...
        x_imag = x_imag + self.pos_embed_imag[:, 1:, :]     # 和位置编码相加
        ...
        x_abs = x_abs + self.pos_embed_abs[:, 1:, :]    # 和位置编码相加
...
if __name__ == '__main__':
    ...
    input = torch.rand(batch_size, 113, 100)  # 输入序列
